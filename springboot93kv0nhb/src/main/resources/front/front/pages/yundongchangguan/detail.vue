	...
	<view class="content">
		<view class="container" :style='{"width":"100%","padding":"0","position":"relative","background":"#fff","height":"100%"}'>
			...
			<view :style='{"width":"100%","padding":"24rpx","background":"#fff","height":"auto"}' class="detail-content">
				<view :style='{"margin":"10rpx 0","borderColor":"#ccc","alignItems":"center","flexWrap":"wrap","borderWidth":"0","display":"flex","width":"100%","borderStyle":"solid","justifyContent":"space-between","height":"auto"}' class="detail-list-item price">
				...
				<view class="detail-list-item" :style='{"margin":"10rpx 0","borderColor":"#ccc","borderWidth":"0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
					<view class="lable" :style='{"width":"160rpx","padding":"0 20rpx 0 0","lineHeight":"60rpx","fontSize":"28rpx","color":"#333","textAlign":"right"}'>时间段：</view>
					<view class="text" :style='{"padding":"0px","margin":"0px","lineHeight":"60rpx","fontSize":"28rpx","color":"rgb(0, 0, 0)"}' >{{detail.shijianduan}}</view>
				</view>
				<view class="detail-list-item" :style='{"margin":"10rpx 0","borderColor":"#ccc","borderWidth":"0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
					<view class="lable" :style='{"width":"160rpx","padding":"0 20rpx 0 0","lineHeight":"60rpx","fontSize":"28rpx","color":"#333","textAlign":"right"}'>场馆状态：</view>
					<view class="text" :style='{"padding":"0px","margin":"0px","lineHeight":"60rpx","fontSize":"28rpx","color":"rgb(0, 0, 0)"}' >{{detail.changguanzhuangtai}}</view>
				</view>
				<view class="detail-list-item" :style='{"margin":"10rpx 0","borderColor":"#ccc","borderWidth":"0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
				...
				<view class="detail-list-item" :style='{"margin":"10rpx 0","borderColor":"#ccc","borderWidth":"0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
					<view class="lable" :style='{"width":"160rpx","padding":"0 20rpx 0 0","lineHeight":"60rpx","fontSize":"28rpx","color":"#333","textAlign":"right"}'>场馆介绍：</view>
					<view class="text" :style='{"padding":"0px","margin":"0px","lineHeight":"60rpx","fontSize":"28rpx","color":"rgb(0, 0, 0)"}'>{{detail.changguanjieshao}}</view>
				</view>
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"width":"100%","padding":"10rpx 0","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}'>
					...
					<button :style='{"border":"2rpx solid #7C6047","padding":"0 20rpx","margin":"0 10rpx 10rpx 0","color":"#7C6047","borderRadius":"8rpx","background":"none","width":"auto","fontSize":"26rpx","lineHeight":"60rpx","height":"60rpx"}' v-if="userid&&isAuth('yundongchangguan','场馆预订')" @tap="onAcrossTap('changguanyuding','','','changguanzhuangtai','已经预约了',changguanzhuangtaiCrossSelect[0])" class="cu-btn bg-brown round shadow-blur" >场馆预订</button>
					<button :style='{"border":"2rpx solid #7C6047","padding":"0 20rpx","margin":"0 10rpx 10rpx 0","color":"#7C6047","borderRadius":"8rpx","background":"none","width":"auto","fontSize":"26rpx","lineHeight":"60rpx","height":"60rpx"}' v-if="!userid&&isAuthFront('yundongchangguan','场馆预订')" @tap="onAcrossTap('changguanyuding','','','changguanzhuangtai','已经预约了',changguanzhuangtaiCrossSelect[0])" class="cu-btn bg-brown round shadow-blur" >场馆预订</button>
					<button :style='{"border":"2rpx solid #7C6047","padding":"0 20rpx","margin":"0 10rpx 10rpx 0","color":"#7C6047","borderRadius":"8rpx","background":"none","width":"auto","fontSize":"26rpx","lineHeight":"60rpx","height":"60rpx"}' v-if="userid&&isAuth('yundongchangguan','私聊')" @tap="chatClick">联系TA</button>
					<button :style='{"border":"2rpx solid #7C6047","padding":"0 20rpx","margin":"0 10rpx 10rpx 0","color":"#7C6047","borderRadius":"8rpx","background":"none","width":"auto","fontSize":"26rpx","lineHeight":"60rpx","height":"60rpx"}' v-if="!userid&&isAuthFront('yundongchangguan','私聊')" @tap="chatClick">联系TA</button>
	...
	export default {
		data() {
			return {
				...
				user: {},
				storeupFlag: 0,
				changguanzhuangtaiCrossSelect:'已预约,未预约'.split(','),
				count: 0,
				timer: null,
				title:'',
			}
		...
		async onShow(options) {
			...
			this.getStoreup();
			let cleanType = uni.getStorageSync('discussyundongchangguanCleanType')
			if(cleanType){
				uni.removeStorageSync('discussyundongchangguanCleanType')
				this.mescroll.num = 1
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('yundongchangguan', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','yundongchangguan');
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								...
								await _this.$api.update('yundongchangguan',_this.detail)
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.add('storeup', {
								userid: _this.user.id,
								name: _this.detail.changguanmingcheng,
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','yundongchangguan');
			...
			async init(type=1){
				...
				let res = await this.$api.info('yundongchangguan', this.id);
				let reg=new RegExp('http://localhost:8080/springboot93kv0nhb/upload','g')//g代表全部
				this.detail = res.data;
				...
				if(type==2){
					...
					await this.$api.update('yundongchangguan',this.detail)
				}
			},
			...
			async upCallback(mescroll) {
				if(uni.getStorageSync("appUserid")){
					let res = await this.$api.list('discussyundongchangguan', {
						page: mescroll.num,
						limit: 10,
			...
			async comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					await this.$api.update('discussyundongchangguan',row)
					this.$utils.msg('点赞成功');
				}else {
					row.thumbsupnum--
					let arr = String(row.tuserids).split(',')
			...
			async onCommentTap() {
				...
				let res = {}
				this.$utils.jump(`../discussyundongchangguan/add-or-update?refid=${this.id}` )
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussyundongchangguan',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('yundongchangguan',that.detail)
							setTimeout(()=>{
