	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussyundongchangguan`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussyundongchangguan`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`discussyundongchangguan`, this.ruleForm);
						}else{
							oet = await this.$api.add(`discussyundongchangguan`, this.ruleForm);
						}
						uni.setStorageSync('discussyundongchangguanCleanType',true);
