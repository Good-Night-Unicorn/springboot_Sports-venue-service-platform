	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusssaishihuodong`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusssaishihuodong`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`discusssaishihuodong`, this.ruleForm);
						}else{
							oet = await this.$api.add(`discusssaishihuodong`, this.ruleForm);
						}
						uni.setStorageSync('discusssaishihuodongCleanType',true);
