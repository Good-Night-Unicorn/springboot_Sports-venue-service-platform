		...
		<view class="container" :style='{"width":"100%","padding":"0","position":"relative","background":"#fff","height":"100%"}'>
			<view :style='{"width":"100%","padding":"24rpx","background":"#fff","height":"auto"}' class="detail-content">
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"width":"100%","padding":"10rpx 0","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}'>
					...
					<button :style='{"border":"2rpx solid #7C6047","padding":"0 20rpx","margin":"0 10rpx 10rpx 0","color":"#7C6047","borderRadius":"8rpx","background":"none","width":"auto","fontSize":"26rpx","lineHeight":"60rpx","height":"60rpx"}' v-if="userid&&isAuth('discusssaishihuodong','私聊')" @tap="chatClick">联系TA</button>
					<button :style='{"border":"2rpx solid #7C6047","padding":"0 20rpx","margin":"0 10rpx 10rpx 0","color":"#7C6047","borderRadius":"8rpx","background":"none","width":"auto","fontSize":"26rpx","lineHeight":"60rpx","height":"60rpx"}' v-if="!userid&&isAuthFront('discusssaishihuodong','私聊')" @tap="chatClick">联系TA</button>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusssaishihuodongCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusssaishihuodongCleanType')
				this.mescroll.num = 1
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('discusssaishihuodong', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','discusssaishihuodong');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','discusssaishihuodong');
			...
			async init(type=1){
				...
				let res = await this.$api.info('discusssaishihuodong', this.id);
				let reg=new RegExp('http://localhost:8080/springboot93kv0nhb/upload','g')//g代表全部
				this.detail = res.data;
				...
				if(type==2){
					...
					await this.$api.update('discusssaishihuodong',this.detail)
				}
			},
			...
			async upCallback(mescroll) {
				if(uni.getStorageSync("appUserid")){
					let res = await this.$api.list('discussdiscusssaishihuodong', {
						page: mescroll.num,
						limit: 10,
			...
			async comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					await this.$api.update('discussdiscusssaishihuodong',row)
					this.$utils.msg('点赞成功');
				}else {
					row.thumbsupnum--
					let arr = String(row.tuserids).split(',')
			...
			async onCommentTap() {
				...
				let res = {}
				this.$utils.jump(`../discussdiscusssaishihuodong/add-or-update?refid=${this.id}` )
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscusssaishihuodong',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discusssaishihuodong',that.detail)
							setTimeout(()=>{
