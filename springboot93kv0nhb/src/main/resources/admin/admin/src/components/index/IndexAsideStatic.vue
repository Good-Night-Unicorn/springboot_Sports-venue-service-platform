<template>
	<div class="menu-preview">
		...
		<el-menu :default-active="activeMenu" :unique-opened="true" :style='{"border":0,"padding":"0","listStyle":"none","margin":"0","position":"relative","background":"transparent"}' mode="horizontal" class="el-menu-horizontal-2">
		    <el-menu-item class="home" :style='{"border":"0","padding":"0","listStyle":"none","margin":"0","color":"#6F6F6F","background":"transparent","float":"left"}' @click.native="menuHandler('')" index="/">
				<div class="el-tooltip">
					...
					<span :style='{"color":"inherit","verticalAlign":"middle","fontSize":"14px"}' slot="title">系统</span>
				</div>
			</el-menu-item>
		    <el-submenu class="user" popper-class="user" :popper-append-to-body="false" :style='{"border":"0","padding":"0","listStyle":"none","margin":"0","color":"#6F6F6F","background":"transparent","float":"left"}' index="1">
				<template slot="title">
					...
					<span :style='{"color":"inherit","verticalAlign":"middle","fontSize":"14px"}' slot="title">用户中心</span>
				</template>
			...
			<template v-for="(menu,index) in menuList.backMenu" v-if="menu.child.length&&menu.child[0].tableName!='hasBoard'">
				<el-submenu v-if="menu.child.length > 1 || !horizontalIsMultiple" class="other" popper-class="other" :popper-append-to-body="false" :style='{"border":"0","padding":"0","listStyle":"none","margin":"0","color":"#6F6F6F","background":"transparent","float":"left"}' :index="index+2+''">
					<template slot="title">
				...
				<el-menu-item v-if="menu.child.length <= 1 && horizontalIsMultiple" class="other" popper-class="other" :style='{"border":"0","padding":"0","listStyle":"none","margin":"0","color":"#6F6F6F","background":"transparent","float":"left"}' @click="menuHandler(menu.child[0].tableName)" :index="'/'+menu.child[0].tableName">
					<div class="el-tooltip">
						<i :style='{"verticalAlign":"middle","margin":"0 3px","fontSize":"18px","color":"inherit","textAlign":"center"}' class="el-icon-menu" :class="icons[index]"></i>
...
export default {
	...
	mounted() {
		...
		this.$http({
			...
			if (data && data.code === 0) {
				if(sessionTable == 'changguanguanli') {
	...
	methods: {
		...
		styleChange() {
			this.$nextTick(() => {
				  ...
				  const icon = {"border":"none","background":"#000000"}
